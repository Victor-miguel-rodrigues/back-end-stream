...
class AuthService {
    ...
    async login(email, senha, ip, userAgent) {
        ...
        if (!validarEmail(email)) {
            throw new types_1.AppError("Email inválido", 400);
        }
        ...
        return await (0, connection_1.transaction)(async (client) => {
            // 1. Buscar usuário
            const usuarioResult = await client.query(`SELECT id, nome_usuario, email, senha_hash, ativo 
                 FROM usuarios 
                 WHERE email = $1`, [email]);
    ...
    async logout(token) {
        return await (0, connection_1.transaction)(async (client) => {
            const sessaoResult = await client.query(`SELECT usuario_id, perfil_id, ip 
                 FROM sessoes 
    ...
    // ============================================
    async podeLogar(email) {
        const result = await (0, connection_1.query)(`SELECT u.id, u.nome_usuario, u.email, p.nome AS perfil, p.limite_usuarios_logados
             FROM usuarios u
        ...
        const vagasResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT s.usuario_id) as logados
             ...
             AND s.ativo = TRUE 
             AND s.data_expiracao > NOW()`, [usuario.id]);
        const logados = parseInt(vagasResult.rows[0].logados);
